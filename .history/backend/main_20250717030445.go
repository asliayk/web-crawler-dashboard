...
func startCrawl(db *sql.DB, id int) {
	...
	log.Printf("startCrawl: Got URL %s for ID %d", rawURL, id)

	log.Printf("startCrawl: Starting Crawl function for %s", rawURL)
	res, err := Crawl(rawURL)
	if err != nil {
		log.Printf("Crawl error for %s: %v", rawURL, err)
		db.Exec("UPDATE urls SET status='error' WHERE id=?", id)
		return
	}
	log.Printf("startCrawl: Crawl completed successfully for %s", rawURL)
	...
	if err != nil {
		log.Printf("startCrawl update results for ID %d: %v", id, err)
		return
	}
	log.Printf("startCrawl: Successfully updated results for ID %d", id)
}
