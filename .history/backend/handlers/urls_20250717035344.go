...
func RegisterURLRoutes(router *gin.Engine, dbConn *sql.DB) {
	...
	api.POST("/urls", func(c *gin.Context) {
		...
		go func(urlID int, url string) {
			...
			log.Printf("BrokenLinkDetails count: %d, brokenLinks count: %d", len(res.BrokenLinkDetails), len(brokenLinks))
			log.Printf("InsertBrokenLinks input: %+v", brokenLinks)
			err = db.InsertBrokenLinks(dbConn, int(urlID), brokenLinks)
			if err != nil {
