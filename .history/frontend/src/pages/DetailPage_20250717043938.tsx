...
interface UrlDetail {
  ...
  broken_links: number | BrokenLink[]
...
export interface BrokenLink {
  ...
  url_id: number
  link: string
  status: number | string // backend might send number; keep flexible
}
...
export default function DetailPage() {
  ...
  const loadBrokenLinks = async () => {
    ...
    try {
      const res = await axios.get<{ broken_links: BrokenLink[] }>(`/urls/${id}/broken`)
