...
export default function DashboardPage() {
  ...
  const [selection, setSelection] = useState<{ ids: (string | number)[] }>({ ids: [] });
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: 24 }}>
      ...
      <BulkActionBar
        ...
        onDelete={() => bulkDelete(selection.ids).then(load)}
        onRestart={() => bulkRestart(selection.ids).then(load)}
      />
      <DashboardTable
        rows={urls}
        onRowClick={(row) => {/* detay modal aç */}}
        selectionModel={selection.ids}
        onSelectionModelChange={(ids) => setSelection({ ids: Array.isArray(ids) ? ids : [] })}
        page={page}
        pageSize={pageSize}
        rowCount={rowCount}
