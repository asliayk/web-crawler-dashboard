...
export default function DashboardPage() {
  ...
  async function fetchData() {
    setLoading(true)
    try {
     const res = await axios.get<{ urls: UrlSummary[] }>('/api/urls')
     console.log('⟵ API /api/urls response:', res.status, res.data)
     // guard against null or undefined
     setData(res.data.urls ?? [])
    } catch (err: any) {
     console.error('Error fetching /api/urls:', err.response?.status, err.response?.data || err.message)
