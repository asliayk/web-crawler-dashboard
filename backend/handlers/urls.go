...
// RegisterURLRoutes sets up all API routes for URL crawling and management.
func RegisterURLRoutes(router *gin.Engine, dbConn *sql.DB) {
	api := router.Group("/api")

	// Create a new URL and start crawling it asynchronously.
	api.POST("/urls", func(c *gin.Context) {
		...
		// Start crawling in a background goroutine
		go func(urlID int, url string) {
			...
			// Update crawl results in the database
			...
			// Remove old broken links and insert new ones
	...
	// List URLs with pagination, sorting, and filtering.
	...
	// Get details for a single URL by ID.
	...
	// Get broken link details for a specific URL.
	...
	// Update the URL string for a given record.
	...
	// Delete a URL and its associated broken links.
	...
	// Bulk delete URLs and their broken links.
	...
	// Bulk restart (re-crawl) multiple URLs asynchronously.
	...
	// Restart (re-crawl) a single URL asynchronously.
